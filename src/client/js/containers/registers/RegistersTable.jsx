...
class RegistersTable extends Component {
  ...
  static mapDispatchToProps(dispatch) {
    return {
      ...
      disableRegister: (username, status = false, name = "") =>
        dispatch(disableRegister(username, status, name)),
    };
  }
      ...
      return (
        <tr key={regitser.username}>
          ...
          <td className="text-nowrap">
            {toFormatedString(regitser.date_registration)}
          </td>
          <td className="text-nowrap">
            {toFormatedString(regitser.date_last_update)}
          </td>
          <td className="text-nowrap">{regitser.name_organization}</td>
          <td className="text-nowrap">{regitser.num_certificate}</td>
          <td className="text-nowrap">
            {regitser.status ? "Активований" : "Деактивований"}
          </td>

          <td className="text-nowrap">
            <form
              className="mx-auto"
              style={{ display: regitser.status ? "inline-flex " : "flex" }}
            ...
            >
              ...
              <button
                className={`btn mx-auto btn-outline-${
                  regitser.status ? "danger" : "primary"
                } my-1 mx-1 btn-sm`}
                data-toggle="modal"
                data-target={`#modalDialog-${regitser.username}`}
                type="button"
              >
                {regitser.status ? "Деактивувати" : "Активувати"}
              </button>
              <ModalDialog
                keyId={regitser.username}
                dangerColor={regitser.status}
                isFetching={this.props.registers.isFetching}
                titleModal={`Підтвердження ${
                  regitser.status ? "де" : ""
                }активації`}
            ...
            {regitser.status && (
              <>
                <Link
                  className={`btn btn-outline-primary my-1 mx-1 btn-sm`}
                  to={`/registers/edit/${regitser.username}`}
                  role="button"
                >
                  Редагувати
                </Link>
                <Link
                  className={`btn btn-outline-primary my-1 mx-1 btn-sm`}
                  to={`/registers/update/${regitser.username}`}
                  role="button"
                >
                  Оновити
                </Link>
  ...
  showRowsOfUsers() {
    ...
    if (this.state.username.length > 4) {
      registers = registers?.filter((x) =>
        x.username.toLowerCase().includes(this.state.username.toLowerCase())
      );
    }
    if (this.state.status != null && this.state.status !== "будь-який статус") {
      registers = registers?.filter(
        (x) => x.status === (this.state.status === "активований")
      );
    }
    if (this.state.dateReg != "") {
      registers = registers?.filter((x) =>
        sameDay(new Date(x.date_registration), new Date(this.state.dateReg))
      );
    }
    if (this.state.dateUpd != "") {
      registers = registers?.filter((x) =>
        sameDay(new Date(x.date_registration), new Date(this.state.dateUpd))
      );
    }

    if (
      this.props.registers.registersObject === null &&
      !this.props.registers.error
    ) {
      ...
      Array.isArray(this.props.registers.registersObject) &&
      registers.length === 0
    ) {
...
export default withRouter(
  connect(
    RegistersTable.mapStateToProps,
    RegistersTable.mapDispatchToProps
  )(RegistersTable)
);
