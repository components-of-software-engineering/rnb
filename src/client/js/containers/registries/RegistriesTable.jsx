...
import { isAdministrator as checkAdminRole, isRegister } from '../../utils/service';
...
class RegistriesTable extends Component {
    ...
    registryRow(registries, isAdministrator) {
    ...
    singleRowInfo(info, isAdministrator) {
        return (
            <tr>
                <td colSpan={isAdministrator ? 4 : 3 } className="text-center">
    ...
    showRowsOfRegistries() {
        ...
        const isAdministrator = checkAdminRole(user.role);
        if (this.props.registries.registriesObject === null && !this.props.registries.error) {
            return this.singleRowInfo("Завантаження даних", isAdministrator);
        } else if (this.props.registries.error) {
            return this.singleRowInfo("Сталася помилка під час завантаження", isAdministrator);
            ...
            return this.registryRow(this.props.registries.registriesObject.data, isAdministrator);
        } else {
            return this.singleRowInfo("Завантаження даних", isAdministrator);
        }
    }
    ...
    textInfo(registries, searchQuery, isAdministrator) {
        if (searchQuery && searchQuery.trim()) {
            return (
                <React.Fragment>
                    <p>
                        Результати пошуку{!isAdministrator && " моїх"} реєстрів за запитом 
                        `<b>{searchQuery}</b>` по опису та назві.
                    </p>
                    {registries.data.length > 0 ?
                        <p>
                            Всього знайдено <b>{registries.totalCount}</b> реєстр(-а)(-ів) за 
                            наданим пошуковим запитом{!isAdministrator && " у моєму розпорядженні"}. На даній сторінці показано 
                        ...
                        :
                        <p>
                            За даним пошуковим запитом{!isAdministrator && "серед моїх ТТН"} нічого не знайдено :(
                        </p>
                    }
                </React.Fragment>
            );
                ...
                <React.Fragment>
                    {registries.data.length > 0 ?
                        <p>
                            ...
                            <b> {registries.totalCount}</b> доступного(-их) на даний момент{!isAdministrator && " у особистому розпорядженні"}.
                        </p>
                        :
                        <p>
                            На жаль, на даний момент немає{!isAdministrator && " моїх"} реєстрів накладних.
                        </p>
                    }
                </React.Fragment>
            );
        ...
        return (
            ...
            <div className="pagination-wrapper"> 
                <div className="table-responsive p-3">
                    <table className={`mx-auto mb-2 styled ${this.props.user.userObject && isRegister(this.props.user.userObject.role) && "shorter"}`} id="registries" >
                        <thead>
                            <tr>
                                <th>Номер</th>
                                <th>Назва</th>
