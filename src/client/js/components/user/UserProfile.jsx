...
import ModalDialog from '../partials/modals/ModalDialog';
...
class UserProfile extends Component {
    ...
    viewIconAdmin(role) {
        if (isAdministrator(role)) {
            return <span className="def"><FontAwesomeIcon icon={faStar} className="ml-3" /></span>;
        }
        return false;
    }
    ...
    viewIconStaff(role) {
        if (isRegister(role)) {
            return <span className="def"><FontAwesomeIcon icon={faUserCog} className="ml-3" /></span>;
        }
        return false;
    }
