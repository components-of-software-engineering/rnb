  ...
  fileInput(props) {
    return (
      <div className="custom-file">
        ...
        <label
          ref={props.labelRef}
          className={`custom-file-label small ${
            props.formInline ? "mx-sm-3" : null
          }`}
          id="file-label"
  ...
  selectInput(props) {
    return (
      <select
        ...
        onChange={props.valueOnChage}
        defaultValue={props.value || "default-type"}
        className="form-control mx-sm-3 custom-select"
        id={`${props.name}_field`}
        required
      >
        {props.optionNotSelectedText && (
          <option hidden disabled value="default-type">
            {props.optionNotSelectedText}
          </option>
        )}
        {props.options &&
            ...
            return (
              <optgroup key={index} label={option.label}>
                {option.value.map((val) => {
                  return (
                    <option key={val.selectValue} value={val.selectValue}>
                      {val.name}
                    </option>
                  );
                })}
