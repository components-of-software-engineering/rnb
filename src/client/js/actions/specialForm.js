...
export function getMinimalInfoAboutSpecialForm(serial, number) {
  return async function (dispatch) {
    ...
    if (response.error !== null) {
      showMessage(
        `Трапилась поимлка ${response.error.message}`,
        typesMessages.error
      )(dispatch);
      return dispatch({
        type: SPECIAL_FORM_GET_MIN_FAILURE,
...
export function createSpecialForm(formData) {
  ...
  return async function (dispatch) {
    ...
    showMessage(
      "Успішно додано звіт витрачання",
      typesMessages.success
    )(dispatch);
  };
}
...
export function createInvoice(formData) {
  ...
  return async function (dispatch) {
    ...
    if (response.error !== null || response.statusCode !== 201) {
      showMessage(
        "Не вдалося створити. Перевірте правильність даних",
        typesMessages.error
      )(dispatch);
      return dispatch({
        type: SPECIAL_FORM_CREATE_FAILURE,
        payload: {
          ...defaultPayload,
          error: response.error ? response.error.message : "error ocurred",
        },
      });
    }
    dispatch({
...
export function updateInvoice(number, formData) {
  ...
  return async function (dispatch) {
    ...
    if (response.error !== null) {
      showMessage(
        "Не вдалося оновити. Перевірте правильність даних",
        typesMessages.error
      )(dispatch);
      return dispatch({
...
export function deleteInvoiceByNum(number) {
  ...
  return async function (dispatch) {
    ...
    if (response.statusCode === 204) {
      ...
      showMessage(
        `Накладна #${number} була успішно видалена`,
        typesMessages.success
      )(dispatch);
      return dispatch({
        type: CURRENT_PATH_REDIRECT,
    ...
    showMessage(
      `Трапилась помилка ${response.error && response.error.message}`,
      typesMessages.error
    )(dispatch);
    return dispatch({
